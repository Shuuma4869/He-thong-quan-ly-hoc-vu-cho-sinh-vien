...
const metrics = [
  ...
  { label: "Kỳ thi sắp tới", value: "—", note: "Không có dữ liệu thật ở bootstrap", icon: CalendarClock },
];
  ...
  return (
    <div className="space-y-8">
      <section className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-sm font-medium text-primary">Tổng quan học vụ</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Chào mừng đến với AMS</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Dashboard hiện chỉ là giao diện nền. Các chỉ số sẽ xuất hiện sau khi người dùng kết nối nguồn dữ liệu.</p></div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <article className="rounded-2xl border bg-card p-6 shadow-sm">
          ...
          <div className="mt-6 grid min-h-48 place-items-center rounded-xl border border-dashed bg-background/60 px-6 text-center"><div><CalendarClock className="mx-auto size-8 text-muted" /><p className="mt-3 text-sm font-medium">Chưa có sự kiện</p><p className="mt-1 text-xs text-muted">AMS không tạo dữ liệu học vụ giả.</p></div></div>
        </article>
        <article className="rounded-2xl border bg-card p-6 shadow-sm">
